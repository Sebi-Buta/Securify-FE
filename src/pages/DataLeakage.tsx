			...
			<Card className="border-0">
				<CardContent className="p-6">
					...
					<p className="text-sm text-muted-foreground">
						Analizează un dump de bază de date scursă pentru a găsi informații sensibile ale utilizatorilor.
					</p>
				</CardContent>
			</Card>
						...
						<TableBody>
							{users.map((u) => (
								<TableRow key={u.id}>
									...
									<TableCell className="font-mono text-xs text-primary truncate max-w-[180px]">
										{u.hash}
									</TableCell>
									<TableCell>
										<span className={`text-sm font-medium ${statusColor(u.status)}`}>
											{u.status}
										</span>
									</TableCell>
								</TableRow>
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
				<Card>
					...
					<CardContent className="space-y-3">
						<p className="text-sm text-muted-foreground">
							Aceste parole sunt hash-uite folosind MD5 (un algoritm învechit și vulnerabil). Poți sparge parola
							adminului?
						</p>
						<div className="flex gap-2">
							<Input
								placeholder="Introdu parola spartă a adminului"
								value={answer}
								onChange={(e) => setAnswer(e.target.value)}
								onKeyDown={(e) => e.key === "Enter" && verify()}
							/>
							<Button onClick={verify} className="shrink-0">
								Verifică
							</Button>
						</div>
					</CardContent>
